...
import {
    ...
    EnableOtpOptionUseCase,
    VerifyOtpLoginUseCase,
    VerifyBackupCodeUseCase,
    DisableOtpOptionUseCase,
} from '../../application/usecases';
import { UserRepositoryDatabase } from '../../infrastructure/repositories/user-repository';
import { OtpBackupCodeRepositoryDatabase } from '../../infrastructure/repositories/otp-backup-code.repository';
...
const enableOtpOptionUseCase = new EnableOtpOptionUseCase(userRepository, backupCodeRepository);
...
const disableOtpUseCase = new DisableOtpOptionUseCase(userRepository, backupCodeRepository);
...
export const enableOtpOption = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const user = (req as Request & { user?: UserPayload }).user;
        if (!user) {
            return res.status(401).jsonError('Vous n\'êtes pas authentifié.');
        ...
        const result = await enableOtpOptionUseCase.execute({
            userId: user.id,
            otpToken,
        });
        res.jsonSuccess(result);
